...
## Меню настроек, Requester, JA3

Тулбар компактный: **Requester · Clear · Export .saz · ⚙ Settings**. Всё
...
**Requester** (кнопка в тулбаре) — компоновщик запросов: метод, URL, заголовки
(построчно `Name: Value`) и тело. **Send** отправляет запрос наверх через тот же
движок с текущим отпечатком; результат появляется в списке как обычная сессия и
сразу открывается. Если в списке выбрана сессия, Requester подставляет её
метод/URL/заголовки/тело — удобно реплеить пойманные запросы.
...
- **Контекстное меню** (ПКМ по строке): копировать URL, копировать как **cURL**,
  повторить в **Requester**, удалить сессию. Кнопка **cURL** есть и в шапке панели
  Request.
...
| POST | `/api/compose` | `{ method, url, headers, body }` — отправить запрос из Requester |
