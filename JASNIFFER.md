...
- **JA3 / отпечаток апстрима** — выпадающий список: Chrome 148 / Chrome 146 /
  Edge / Firefox / Safari. Смена пересоздаёт апстрим‑клиентов и меняет
  User‑Agent + client‑hints и **HTTP/2‑отпечаток** (порядок псевдозаголовков,
  SETTINGS). *Замечание о точности:* TLS‑слой (сам JA3 ClientHello) определяется
  нативной библиотекой tls‑client; в текущей сборке движка ClientHello между
  пресетами не различается (это ограничение движка, не снифера — движок по
  условию не переписывается).
