...
- **Smart Redirects**, **Capture**, **UDP** — тумблеры‑свитчи;
...
## Кастомный отпечаток (захват JA3/JA4 браузера)

Кроме встроенных пресетов можно **снять настоящий отпечаток своего браузера** и слать
апстрим‑запросы именно с ним. **⚙ Settings → «Кастомный отпечаток» → «Снять отпечаток
браузера»** открывает локальную HTTPS‑страницу захвата (`https://localhost:8867/`, порт
переопределяется `JASniffer__CapturePort`). Откройте её **в том браузере, чей отпечаток
нужен** (требуется установленный корневой CA) — страница соединяется **напрямую** (не
через MITM‑прокси), поэтому снимается **подлинный ClientHello браузера**.

Снифер парсит ClientHello в:

- **JA3** (+ MD5) и **JA4** — для отображения;
- **полную спецификацию `CustomTlsClient`** для движка: JA3‑строка с сохранёнными
  GREASE‑позициями, `supportedVersions`, `keyShareCurves`, `supportedSignatureAlgorithms`,
  `alpnProtocols`, `certCompressionAlgos` — числовые id маппятся в точную номенклатуру
  `bogdanfinn/tls-client`, поэтому отпечаток **воспроизводится дословно** (проверено
  round‑trip‑тестом: снимок ClientHello движка → спецификация → повторная отправка →
  совпадение JA3).

Захваченные отпечатки сохраняются (`fingerprints.json` рядом с CA), показываются списком;
кнопка **«Использовать»** применяет выбранный к апстрим‑легу (пересоздаёт клиентов, как
смена пресета), повторное нажатие — сбрасывает. Активный отпечаток восстанавливается при
старте.

> **Объём.** Захватывается и воспроизводится **TLS‑слой** (JA3/JA4 + содержимое
> расширений). **HTTP/2‑отпечаток** (Akamai: SETTINGS/WINDOW_UPDATE/порядок
> псевдо‑заголовков) пока берётся из ближайшего пресета — браузер↔страница захвата идёт по
> HTTP/1.1, снятие h2‑отпечатка требует h2‑сервера захвата (план).
...
- **HTTP/3 (QUIC).** Многие CDN (CloudFront, Google и т.п.) отдают
  ...
  CONNECT именно потому, что глушит QUIC.
...
**Решение — не расшифровывать такой хост:** хост пойдёт **сырым туннелем** — браузер
соединится с Cloudflare **напрямую** (свой настоящий TLS/h2/h3), челлендж решится,
сайт заработает.

**Автоматически (по умолчанию вкл).** Тумблер **⚙ Settings → «Авто-байпас
Cloudflare-челленджа»**: снифер сам ловит челлендж (заголовок `cf-mitigated`, либо
`403/503` от Cloudflare с challenge-platform скриптом в теле), помечает сессию
бейджем **«CF challenge → bypass»** и **на лету** добавляет хост (с поддоменами) в
рантайм-список сырых туннелей. Дальше — **перезагрузите вкладку**: соединение к этому
хосту пойдёт напрямую, и капча решится браузером сама. Авто-список держится **в
памяти** и сбрасывается при перезапуске (чтобы разовый челлендж не выключал инспекцию
хоста навсегда). Тумблер можно отключить — тогда детект остаётся (бейдж), но
авто-туннель не включается.

**Вручную.** ПКМ по сессии → **«Не расшифровывать этот хост (bypass)»** (или впишите
хост в **⚙ Settings → «Пропускать без расшифровки»**) — постоянный, персистится между
запусками. Забайпасенные сессии видны в списке как **tunnel**.
...
- **Удаление одной сессии** (`Del` или меню) — не только полная очистка.
- **Горячие клавиши:** `↑`/`↓` — навигация, `Del` — удалить выбранную, `/` или
  `Ctrl+F` — фокус в фильтр, `Esc` — закрыть модалку/меню.
...
| GET | `/api/fingerprints` | список захваченных отпечатков + URL страницы захвата + активный id |
| POST | `/api/fingerprints/active` | `{ id }` — применить захваченный отпечаток к апстриму (пустой id — сбросить) |
| DELETE | `/api/fingerprints/{id}` | удалить захваченный отпечаток |
