...
- **JA3 / отпечаток апстрима** — выпадающий список: Chrome 148 / Chrome 146 /
  Edge / Firefox / Safari. Смена пересоздаёт апстрим‑клиентов и меняет реальный
  **TLS ClientHello** (JA3), **HTTP/2‑отпечаток** и User‑Agent/client‑hints. Рядом
  кнопка **«Проверить отпечаток»** — снимает настоящий ClientHello движка **по
  loopback** и показывает JA3, число шифров/расширений, TLS 1.3/key_share/GREASE.
...
- **Апстрим‑прокси (исходящий)** — весь исходящий (апстрим) трафик идёт через
  ...
  `https://…`, `socks5://ip:port`, `ip:port`, `ip:port:user:pass`. Кнопка **Тест**
  делает пробный запрос и показывает внешний IP. Чекбокс **Ротационный** — для
  прокси, меняющих exit‑IP на каждый запрос (отключает пул соединений против EOF).
  Прокси меняется «на ходу» (cookie/пул сессии сохраняются).
